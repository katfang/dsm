...
void strassen_boxed(void *a, struct task *t) {
  ...
  dsm_free(args, sizeof(struct strassen_args));
}
...
double **allocate_real_matrix(int tam, int random) {
  ...
  v = (double**) dsm_malloc(n * sizeof(double*));
  ...
  for (i = 0; i < n; i++) {
    v[i] = (double*) dsm_malloc(m * sizeof(double));
...
double **free_real_matrix(double **v, int tam) {
  ...
  for (i = 0; i < tam; i++) { 
    if (v[i]) { 
      dsm_free(v[i], sizeof(double) * tam); // frees a row of the matrix
      v[i] = NULL;
    } 
  } 
 
  dsm_free(v, sizeof(double *) * tam);     // frees the pointer /
  v = NULL;
